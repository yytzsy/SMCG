...
class LanguageModelCriterion_withSyntax(nn.Module):
    ...
    def forward(self, logits_caption, target_caption, mask_caption, logits, target, mask, rec_fts, video_fts):
        """
        logits: shape of (N, seq_len, vocab_size)
        target: shape of (N, seq_len)
        mask: shape of (N, seq_len)
        ...
        rec_fts = rec_fts.contiguous().view(-1, video_fts_dim)
        video_fts = video_fts.contiguous().view(-1, video_fts_dim)
