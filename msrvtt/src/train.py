...
def train(loader, model, crit, optimizer, lr_scheduler, opt, rl_crit=None):
    ...
    for epoch in range(opt["epochs"]):
        ...
        for data in loader:
            ...
            loss, loss_syntax, loss_content, loss_caption = crit(seq_probs, labels[:, 1:], masks[:, 1:], seq_prob_syntax, parse_labels[:,1:], parse_masks[:,1:], reconstruct_video_sequence, fc_feats)
            ...
            if not sc_flag:
                print("iter %d (epoch %d), train_loss = %.6f, train_loss_syntax = %.6f, train_loss_content = %.6f, train_loss_caption = %.6f" %
                      (iteration, epoch, train_loss, train_loss_syntax, train_loss_content, train_loss_caption))
            else:
        ...
        if epoch % opt["save_checkpoint_every"] == 0:
            ...
            with open(model_info_path, 'a') as f:
                f.write("model_%d, loss: %.6f\t" % (epoch, train_loss))
